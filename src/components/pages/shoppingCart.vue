...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      //套用優碼
      getDiscount() {
        const api = `https://vue-course-api.hexschool.io/api/jwcabinet/coupon`;
        const vm = this;
      ...
      delCart(cartItem) {
        const api = `https://vue-course-api.hexschool.io/api/jwcabinet/cart/${cartItem.id}`;
        const vm = this;
      ...
      // 建立訂單
      creatOrder() {
        const api = `https://vue-course-api.hexschool.io/api/jwcabinet/order`;
        const vm = this;
